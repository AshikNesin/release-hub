...
SELECT id, app_id, platform, package_name, play_enabled, sign_keystore, sign_config, sign_sha256, created_at, play_account_id, prune_keep FROM app_platforms WHERE app_id = ? AND platform = ?
`
...
SELECT id, app_id, platform, package_name, play_enabled, sign_keystore, sign_config, sign_sha256, created_at, play_account_id, prune_keep FROM app_platforms WHERE app_id = ? ORDER BY platform
`
...
func (q *Queries) ListReleasesByChannel(ctx context.Context, arg ListReleasesByChannelParams) ([]Release, error) {
	rows, err := q.db.QueryContext(ctx, listReleasesByChannel, arg.AppPlatformID, arg.Channel)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
SELECT ap.id, ap.app_id, ap.platform, ap.package_name, ap.play_enabled, ap.sign_keystore, ap.sign_config, ap.sign_sha256, ap.created_at, ap.play_account_id, ap.prune_keep FROM app_platforms ap JOIN apps a ON a.id = ap.app_id
...
const platformsForPrune = `-- name: PlatformsForPrune :many
SELECT ap.id, ap.app_id, ap.platform, ap.package_name, ap.play_enabled, ap.sign_keystore, ap.sign_config, ap.sign_sha256, ap.created_at, ap.play_account_id, ap.prune_keep FROM app_platforms ap
`

func (q *Queries) PlatformsForPrune(ctx context.Context) ([]AppPlatform, error) {
	rows, err := q.db.QueryContext(ctx, platformsForPrune)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
const setPruneKeep = `-- name: SetPruneKeep :exec
UPDATE app_platforms SET prune_keep = ? WHERE id = ?
`

type SetPruneKeepParams struct {
	PruneKeep *int64 `json:"prune_keep"`
	ID        int64  `json:"id"`
}

func (q *Queries) SetPruneKeep(ctx context.Context, arg SetPruneKeepParams) error {
	_, err := q.db.ExecContext(ctx, setPruneKeep, arg.PruneKeep, arg.ID)
	return err
}
